...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
	...
	elif "Dahoas/template-full-hh-rlhf" in dataset_name:
		return raw_datasets.DahoasFullhhrlhfTemplateDataset(
			output_path, seed, local_rank, dataset_name="Dahoas/full-hh-rlhf"
		)
...
class DeterminatorDataset(Dataset):
	def __init__(
			self, chosen_dataset, reject_dataset, modified_dataset, pad_token_id, train_phase
	):
		super().__init__()
		self.chosen_dataset = chosen_dataset
		self.reject_dataset = reject_dataset
		...
		self.train_phase = train_phase
	def __len__(self):
		length = len(self.reject_dataset)
		return length
	def __getitem__(self, idx):
		if self.train_phase == 2:
			return (
				...
				self.reject_dataset[idx]["input_ids"],
				self.reject_dataset[idx]["attention_mask"],
				self.reject_dataset[idx]["edit_token_label"],
				self.modified_dataset[idx]["input_ids"],
				self.modified_dataset[idx]["attention_mask"],
				self.modified_dataset[idx]["edit_token_label"]
			)
		else:
			import ipdb; ipdb.set_trace()

class DeterminatorEvalDataset(Dataset):
	def __init__(
			self, prompt_dataset, chosen_dataset, reject_dataset, pad_token_id, train_phase
	):
		super().__init__()
	...
	def __getitem__(self, idx):
		if self.train_phase == 2:
			return (
				...
				self.reject_dataset[idx]["input_ids"],
				self.reject_dataset[idx]["attention_mask"],
				self.prompt_dataset[idx]["prompt_length"],
			)
		else:
			import ipdb; ipdb.set_trace()
class DeterminatorTokenEvalDataset(Dataset):
	def __init__(
			self, chosen_dataset, reject_dataset, modified_dataset, prompt_dataset, pad_token_id, train_phase
	):
		super().__init__()
		self.chosen_dataset = chosen_dataset
		self.reject_dataset = reject_dataset
	...
	if train_phase == 1:
		for i, tmp_data in enumerate(current_dataset):
			...
			if chosen_sentence is not None:
				...
				if prompt_length == max_seq_len:
					...
					# if not isinstance(raw_dataset, raw_datasets.DahoasFullhhrlhfTemplateDataset): # added by esyoon 2024-03-17-19:04:15
					labels[:prompt_length] = IGNORE_INDEX # when </s> is not used (template is not applied)  
		...
		for i, tmp_data in enumerate(current_dataset):
			...
			if chosen_sentence is not None and reject_sentence is not None:
				...
				# skip the sample only prompt is contained
				if tokenizer.pad_token_id in chosen_token["input_ids"]:
...
# added by esyoon 2024-01-27-16:49:46
def create_discriminator_dataset(
	...
	for i, tmp_data in enumerate(dataset):
		# tokenize the text
		if custom_template:
			raw_prompt = tmp_data['context'].replace('\n###Human:', '</s>[INST]')
			raw_prompt = raw_prompt.replace('###Human:', '[INST]')
			raw_prompt = raw_prompt.replace('\n###Assistant:', '</s>[/INST]')
			raw_prompt += '</s>[/INST]'
		else:

			raw_prompt = tmp_data['context'].replace('\n###Human', '\n\nHuman')
			raw_prompt = raw_prompt.replace('###Human', '\n\nHuman')
			raw_prompt = raw_prompt.replace('\n###Assistant', '\n\nAssistant')
			raw_prompt += '\n\nAssistant:'
		
		reject_sentence = raw_prompt + ' ' + tmp_data['reject_response']
		modification = raw_prompt + ' ' + tmp_data['response']
		chosen_sentence = raw_prompt + ' ' + tmp_data['chosen_response']
		if reject_sentence is not None and modification is not None and chosen_sentence is not None:
			reject_sentence += end_of_conversation_token
			modification += end_of_conversation_token
			chosen_sentence += end_of_conversation_token
			prompt_token = tokenizer(
				raw_prompt,
				return_tensors="pt",
			...
			# prompt_length = reject_token['attention_mask'].sum().item()-len(tmp_data['rejected_token_edit'])-1
			# prompt_length_ = modification_token['attention_mask'].sum().item()-len(tmp_data['modified_token_edit'])-1

			prompt_length = prompt_token['attention_mask'].sum().item()
			if prompt_length >= max_seq_len:
				continue

			reject_token['edit_token_label'] = reject_token['attention_mask'].clone()
			reject_token['edit_token_label'][0][:prompt_length] = -100
			reject_token['edit_token_label'][0][(reject_token['input_ids'][0]==tokenizer.pad_token_id).nonzero().flatten()] = -100
			reject_label = torch.tensor([1 if i == 'S' or i == 'D' else 0 for i in tmp_data['rejected_token_edit']])
			if prompt_length+len(tmp_data['rejected_token_edit']) > max_seq_len:
				reject_token['edit_token_label'][:, prompt_length:max_seq_len] = reject_label[:max_seq_len-prompt_length]
			else:
				reject_token['edit_token_label'][:, prompt_length:prompt_length+len(tmp_data['rejected_token_edit'])] = reject_label

			modification_token['edit_token_label'] = modification_token['attention_mask'].clone()
			modification_token['edit_token_label'][0][:prompt_length] = -100
			modification_token['edit_token_label'][0][(modification_token['input_ids'][0]==tokenizer.pad_token_id).nonzero().flatten()] = -100
			modification_label = torch.tensor([1 if i == 'S' or i == 'A' else 0 for i in tmp_data['modified_token_edit']])
			if prompt_length+len(tmp_data['modified_token_edit']) > max_seq_len:
				modification_token['edit_token_label'][:, prompt_length:max_seq_len] = modification_label[:max_seq_len-prompt_length]
			else:
				modification_token['edit_token_label'][:, prompt_length:prompt_length+len(tmp_data['modified_token_edit'])] = modification_label

			# skip the sample only prompt is contained
			if tokenizer.pad_token_id in reject_token["input_ids"] and tokenizer.pad_token_id in chosen_token["input_ids"]:
				chosen_token['input_ids'] = chosen_token['input_ids']
				chosen_token['attention_mask'] = chosen_token['attention_mask']
				chosen_dataset.append(chosen_token)
				...
				modification_token["input_ids"] = modification_token["input_ids"]
				modification_token["attention_mask"] = modification_token["attention_mask"]
				modification_dataset.append(modification_token)
			else:
				continue
	return DeterminatorDataset(
		chosen_dataset,
		reject_dataset,
		modification_dataset,
		tokenizer.pad_token_id,
...
def create_discriminator_eval_dataset(
		local_rank, 
		data_path, 
		seed,
		tokenizer,
	...
	chosen_dataset = []
	reject_dataset = []
	assert len(data_path) == 1 # not implemented for multiple dataset
	dataset  = load_dataset(data_path[0], split='test')
	for i, tmp_data in enumerate(dataset):
		# tokenize the text
		raw_prompt = tmp_data['prompt']
		reject_sentence = raw_prompt + tmp_data['rejected']
		chosen_sentence = raw_prompt + tmp_data['chosen']

		if raw_prompt is not None and reject_sentence is not None and chosen_sentence is not None:
			reject_sentence += end_of_conversation_token
			chosen_sentence += end_of_conversation_token
			...
			if tokenizer.pad_token_id in reject_token["input_ids"] and tokenizer.pad_token_id in chosen_token["input_ids"]:
				...
				prompt_token["input_ids"] = prompt_token["input_ids"]
				prompt_token["attention_mask"] = prompt_token["attention_mask"]
				prompt_token['prompt_length'] = prompt_length
				prompt_dataset.append(prompt_token)

			else:
				continue
	return DeterminatorEvalDataset(
		prompt_dataset,
		chosen_dataset,
		reject_dataset,
		tokenizer.pad_token_id,
...
# added by esyoon 2024-01-27-16:49:46
def create_discriminator_token_eval_dataset(
		local_rank, 
		dataset, 
		seed,
		tokenizer,
	...
	for i, tmp_data in enumerate(dataset):
		...
		if reject_sentence is not None and modification is not None and chosen_sentence is not None:
			...
			if tokenizer.pad_token_id in reject_token["input_ids"] and tokenizer.pad_token_id in chosen_token["input_ids"]:
				...
				continue
	return DeterminatorTokenEvalDataset(
		chosen_dataset,
		reject_dataset,
		modification_dataset,
		prompt_dataset,
...
class DataCollatorDiscriminatorReward:
	def __call__(self, data):
		batch = {}
		# chosen, reject, modified
		batch["input_ids"] = torch.cat(
			[f[0] for f in data] + [f[2] for f in data] + [f[5] for f in data], dim=0
		)
		batch["attention_mask"] = torch.cat(
			[f[1] for f in data] + [f[3] for f in data] + [f[6] for f in data], dim=0
		)
		batch['reject_edit_label'] = torch.cat([f[4] for f in data], dim=0)
		batch['modification_edit_label'] = torch.cat([f[7] for f in data], dim=0)
		return batch

class DataCollatorDiscriminatorEvalReward:
	def __call__(self, data):
		batch = {}
		batch["input_ids"] = torch.cat(
			[f[0] for f in data] + [f[2] for f in data], dim=0
...
class DataCollatorDiscriminatorTokenEvalReward:
	def __call__(self, data):
		batch = {}
		batch["input_ids"] = torch.cat(
			[f[0] for f in data] + [f[2] for f in data] + [f[5] for f in data], dim=0
		...
		batch['reject_edit_label'] = torch.cat([f[4] for f in data], dim=0)
		batch['modification_edit_label'] = torch.cat([f[7] for f in data], dim=0)
		batch['prompt_length'] = torch.tensor([f[8] for f in data])
		return batch
class DataCollatorCustomReward:
	def __call__(self, data):
		batch = {}
		batch["input_ids"] = torch.cat(
			[f[5] for f in data] + [f[2] for f in data], dim=0
		)
		batch["attention_mask"] = torch.cat(
			[f[6] for f in data] + [f[3] for f in data], dim=0
		)
		return batch
...
class DataCollatorGeneration:
	...
	def __call__(self, data):
		batch = {}
		pad_token_id = 2 #TODO: change to dynamic value
