...
class DeepSpeedPPOTrainer:
    def __init__(self, rlhf_engine, args):
        ...
        self.alpha_ctl = torch.tensor(0.0)

    def _generate_sequence(
        self, prompts, mask, step, deterministic=False, print_answers=False
    ):
        ...
        if print_answers:
            print(
                f"--- prompt --> step={step}, rank={torch.distributed.get_rank()}, {self.tokenizer.decode(prompts[0], skip_special_tokens=True)}"
            )
            print(
                f"--- ans    --> step={step}, rank={torch.distributed.get_rank()}, {self.tokenizer.decode(ans[0], skip_special_tokens=True)}"
            )
        ...
        if len(out_seq) > 0:
            ...
            return self._generate_sequence(
                prompts, mask, step, deterministic, print_answers
            )
        return out_seq
    ...
    def generate_experience(
        ...
        with torch.no_grad():
            ...
            # added by esyoon 2024-01-29-12:15:20
            reward_dict = self.reward_model.forward_value(
                seq, action_mask, prompt_length=self.prompt_length
            )
            reward_score = reward_dict["reward_scores"].detach()
            token_reward_score = reward_dict["token_reward_scores"].detach()
            holistic_reward_score = reward_dict["token_reward_scores"].detach()
            # holistic_reward_score = reward_dict["chosen_end_scores"].detach()

            # reward_score = self.reward_model.forward_value(
            #     seq, action_mask, prompt_length=self.prompt_length
            # )["reward_scores"].detach() # added by esyoon 2024-01-28-22:30:12
            if eval_mode:
                values = None
            else:
                values = self.critic_model.forward_value(
                    seq, action_mask, return_value_only=True
                ).detach()[:, :-1]
        ...
        return {
            ...
            "value": values,
            "rewards": reward_score[:,:-1 ],
            "full_kl": full_kl,
            "entropy": entropy,
            "input_ids": seq,
            "attention_mask": action_mask,
            "token_reward_scores": token_reward_score[:,:-1 ],
            "holistic_reward_scores": holistic_reward_score,
        }

    def compute_rewards(self, prompts, kl_divergence, reward_score, action_mask):
        if self.penalty.endswith("onestep"):
            rewards = torch.zeros_like(kl_divergence)
        else:
        ...
        for j in range(batch_size):
            try:
                rewards[j, start : ends[j]] += reward_clip[j, start : ends[j]] 
            except:
                ipdb.set_trace()
        return rewards

    def train_rlhf(self, inputs, debug_mode=False):
        ...
        old_values = values
        with torch.no_grad():
            if self.penalty.startswith("kl"):
                kl_divergence = -(log_probs - ref_log_probs)

                condition = ref_log_probs > torch.log(self.alpha_ctl)
                kl_divergence = torch.where(condition, log_probs, log_probs - ref_log_probs)

            elif self.penalty.startswith("full_kl"):
                kl_divergence = -full_kl[:, :-1]
            else:
            ...
            if self.penalty.endswith("onestep"):
                advantages += kl_divergence[:, start:]
    ...
    def get_rewards(self, rewards, attention_mask, start):
        returns = []
        ends = start + attention_mask[:, start:].sum(1) - 1
        for i in range(rewards.shape[0]):
            last_return = 0
            reward_list = []
            if ends[i] > rewards.shape[1] + 1:
                ends[i] = rewards.shape[1] + 1

            for t in reversed(range(start, ends[i])):
                # last_return = rewards[i, t] + self.gamma * last_return
                reward_list.append(rewards[i, t])
            reward_value = torch.stack(reward_list)
            returns.append(reward_value.sum())
            # returns.append(reward_value.mean())
