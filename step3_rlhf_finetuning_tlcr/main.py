...
def parse_args():
    ...
    parser.add_argument(
        "--use_negative_reward_only",
        action="store_true",
    )
    parser.add_argument(
        "--use_positive_reward_only",
        action="store_true",
    )
    parser.add_argument(
        "--holistic_reward",
    ...
    parser.add_argument(
        "--fixed_positive_reward",
        type=float,
        default=1.0,
    )
    parser.add_argument(
        "--fixed_negative_reward",
        type=float,
        default=-1.0,
    )   
    parser.add_argument(
...
def evaluation_by_reward(
    ...
    eval_kl = []
    eval_entropy = []
    eval_token_level_reward = []
    eval_holistic_reward = []
    num_samples = 0
    for step, batch_prompt in enumerate(eval_dataloader):
        ...
        returns = trainer.get_rewards(exp["rewards"], exp["attention_mask"], start = exp['prompts'].size()[-1] - 1)
        token_level_returns = trainer.get_rewards(exp['token_reward_scores'], exp['attention_mask'], start = exp['prompts'].size()[-1] - 1)
        holistic_reward = exp['holistic_reward_scores'].mean()

        reward = returns.mean()
        token_level_reward = token_level_returns.mean()
        ...
        eval_token_level_reward.append(token_level_reward.item())
        eval_holistic_reward.append(holistic_reward.item())
        ...
        if args.save_answers and num_samples <= 100:
            ...
            rewards = [rew.item() for rew in returns]
    ...
    return (
        np.mean(eval_reward),
        np.mean(eval_token_level_reward),
        np.mean(eval_holistic_reward),
...
def main():
    ...
    if torch.distributed.get_rank() == 0:
        if args.enable_tensorboard:
            ...
            wandb.init(project="ReMax",name='llama2-7b_rebuttal_tlcr_discrete_no_thres')
    ...
    eval_reward, eval_token_reward, eval_holistic_reward, eval_length, eval_kl, eval_entropy = evaluation_by_reward(
        trainer,
        prompt_eval_dataloader,
        device,
        args,
    ...
    print_rank_0(
        f"eval reward: {eval_reward} | eval token reward: {eval_token_reward} | eval holistic reward: {eval_holistic_reward} | eval length: {eval_length} | eval kl: {eval_kl} | eval entropy: {eval_entropy} | eval ppl: {perplexity}",
        args.global_rank,
    )
    if args.enable_tensorboard and torch.distributed.get_rank() == 0:
        wandb.log({"global_step": global_step, "eval/reward": eval_reward, "eval/token_reward": eval_token_reward, "eval/holistic_reward": eval_holistic_reward, "eval/length": eval_length, "eval/kl": eval_kl, "eval/entropy": eval_entropy, "eval/ppl": perplexity})
            ...
            if exp_dataset is not None:
                ...
                if args.enable_tensorboard and torch.distributed.get_rank() == 0:
                    wandb.log({"train/reward": average_reward / inner_iter,  "train/return":  average_return / inner_iter, 
                               ...
                               "train/critic_loss": critic_loss.item(), "train/critic_loss_sum": critic_loss_sum,
                ...
                if (global_step + 1) % (len(prompt_train_dataloader) // 20) == 0:
                    ...
                    perplexity = evaluation_by_ppl(trainer, ppl_eval_dataloader, device)
                    (
                        eval_reward, eval_token_reward, eval_holistic_reward,
                        eval_length,
                        eval_kl,
                        eval_entropy,
