...
class DeepSpeedRLHFEngine:
    ...
    def _init_critic(self, critic_model_name_or_path):
        ...
        # Model
        if self.args.discriminate_reward:
            if self.args.uni_discriminator_head:
                critic_model = create_discriminator_holistic_uni_critic_model(
                ...
                critic_model = create_discriminator_w_holistc_critic_model(
                    model_name_or_path=critic_model_name_or_path,
                    tokenizer=self.tokenizer,
                    ds_config=ds_config,
                    num_padding_at_beginning=self.args.num_padding_at_beginning,
            ...
            critic_model = create_critic_model(
                ...
                disable_dropout=self.args.disable_critic_dropout,
                zero_stage=self.args.critic_zero_stage,
                bidir_attn=self.args.bidirectional_reward,
                bidir_layer_num=self.args.bidirectional_layer_num
            )
        ...
        if self.args.use_negative_reward_only:
            critic_model.negative_reward_only = True
        if self.args.use_positive_reward_only:
            critic_model.positive_reward_only = True
        if self.args.holistic_reward:
            critic_model.holistic_reward = True
        if self.args.random_dense_reward:
            critic_model.random_dense_reward = True
        if self.args.fixed_reward:
            critic_model.fixed_reward = True
            critic_model.positive_reward = self.args.fixed_positive_reward
            critic_model.negative_reward = self.args.fixed_negative_reward
        if self.args.sum_to_one_reward:
            critic_model.sum_to_one_reward = True
        # DeepSpeed Engine
    ...
    def _init_reward(self, critic_model_name_or_path):
        ...
        if self.args.discriminate_reward:
            if self.args.holistic_reward:
                reward_model = create_discriminator_w_holistc_critic_model(
                    model_name_or_path=critic_model_name_or_path,
                    tokenizer=self.tokenizer,
                    ds_config=ds_config,
                    num_padding_at_beginning=self.args.num_padding_at_beginning,
            ...
            if self.args.uni_discriminator_head:
                reward_model = create_discriminator_uni_critic_model(
                    model_name_or_path=critic_model_name_or_path,
                    tokenizer=self.tokenizer,
                    ds_config=ds_config,
                    num_padding_at_beginning=self.args.num_padding_at_beginning,
                ...
                reward_model = create_discriminator_critic_model(
                    model_name_or_path=critic_model_name_or_path,
                    tokenizer=self.tokenizer,
                    ds_config=ds_config,
                    num_padding_at_beginning=self.args.num_padding_at_beginning,
        ...
        if self.args.use_negative_reward_only:
            reward_model.negative_reward_only = True
        if self.args.use_positive_reward_only:
            reward_model.positive_reward_only = True
        if self.args.holistic_reward:
            reward_model.holistic_reward = True
        if self.args.random_dense_reward:
            reward_model.random_dense_reward = True
        if self.args.fixed_reward:
            reward_model.fixed_reward = True
            reward_model.positive_reward = self.args.fixed_positive_reward
            reward_model.negative_reward = self.args.fixed_negative_reward
        if self.args.sum_to_one_reward:
            reward_model.sum_to_one_reward = True
