...
def parse_args():
    ...
    ## Tensorboard logging
    parser.add_argument(
        "--enable_tensorboard", action="store_true", help="Enable wandb logging"
    )
...
def main():
    ...
    # added by esyoon 2024-01-09-22:19:24
    if torch.distributed.get_rank() == 0:
        if args.enable_tensorboard:
            #writer = SummaryWriter(
            #    f"{args.tensorboard_path}/step3_tensorboard_logs")
            # wandb.init(project="ReMax",name='opt1.3b_sft_240109_bsz128_hh_rlhf')
            # wandb.init(project="ReMax",name='llama2-7b_sft_bsz16_mask_template_hh_rlhf')
            wandb.init(project="ReMax",name='llama2-13b_sft_bsz16_hh_rlhf')
        else:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if torch.distributed.get_rank() == 0:
                ...
                if args.enable_tensorboard:
                    wandb.log({"sft_train/loss": loss, 'sft_train/step': step})
            ...
            if (step + 1) % int(len(train_dataloader) // 10) == 0:
                ...
                if model.monitor.enabled and model.global_rank == 0:
                    ...
                    # model.monitor.write_events(summary_events)
                    if args.enable_tensorboard:
                        wandb.log({"sft_eval/ppl": perplexity})
                model.train()
